<?php
	/*
		-------------------------------------------------------------------
		New Reflections by Sérgio Serra (a.sergio.serra@gmail.com)
		Fork of Easy Reflections v3 by Richard Davey, Core PHP (rich@corephp.co.uk)
        -------------------------------------------------------------------
		...
		img		        required	The (local) url image to reflect
	...
	*/
	...
	if (isset($_GET['img']))
	{
		...
		$source_image = preg_replace("/^[a-zA-Z]+[:\/\/]+[A-Za-z0-9\-_]+\\.?+[A-Za-z]+\\.[A-Za-z]+/", '',$source_image);
		$source_image = $_SERVER['DOCUMENT_ROOT'] . $source_image;
		
		if(!file_exists($source_image)){ //just in case you are using in localhost
            $source_image = $_GET['img'];
            $source_image = preg_replace("/^[a-zA-Z]+[:\/\/]+[A-Za-z0-9\-_]+\\.?+[localhost]/", '',$source_image);
            $source_image = $_SERVER['DOCUMENT_ROOT'] . $source_image;
		}
